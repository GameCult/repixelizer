...
function queueSummaryFromState(activeStatus: string | null = state.status): QueueSummary {
  return {
...
type StatusMetricItem = {
  label: string;
  value: string;
};

function formatGridValue(width: number | null | undefined, height: number | null | undefined, fallback = "pending"): string {
  if (!width || !height) {
    return fallback;
  }
...
function getInspectReferenceWidth(panels: Record<ViewerPanelKey, ViewerPanelState>): number {
...
async function startRun(): Promise<void> {
  ...
  if (uploadLimit !== undefined && state.file.size > uploadLimit) {
    ...
    setStage(
      "failed",
      "File too large",
      `${state.file.name} is ${formatByteLimit(state.file.size)}. Hosted uploads are capped at ${formatByteLimit(uploadLimit)}.`,
    );
...
async function acceptFile(file: File): Promise<void> {
  ...
  if (uploadLimit !== undefined && file.size > uploadLimit) {
    ...
    setStage(
      "failed",
      "File too large",
      `${file.name} is ${formatByteLimit(file.size)}. Hosted uploads are capped at ${formatByteLimit(uploadLimit)}.`,
    );
    addLog("Failure", "Selected file is over the hosted upload limit.");
    await renderEverything();
    return;
