...
- Removed unused old solver hyperparams from `src/repixelizer/params.py`.
...
- Simplified `cleanup_pixels(...)` to alpha snap plus zero heatmap; the old iterative cleanup path was unreachable.
...
- Updated GUI event listeners from removed `lattice_search_*` events to the live `lattice_inference_*` events.
...
- Updated `docs/spec.md` so it describes the live explicit local-search solver and alpha snapping, not the removed autograd/projected-Adam solver.

## Verification already run
- AST parse across `src`, `scripts`, `tools`, and `tests`: passed.
- Targeted stale-string scan for removed solver params, fake cleanup hooks, old guidance names, and solver-analysis call sites: clean.
